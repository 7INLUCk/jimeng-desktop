...
### BatchCardGrid 高度对齐 + 操作按钮（2026-04-16 完成）

**问题：** 批量卡 info 区无操作行，导致与单任务卡高度差约 45px；Row1 无 min-h，Row3 mt 偏大。

**修复（`WorksPanel.tsx`）：**

- `BatchCardGrid` 转为有状态组件（`showDeleteConfirm` + `folderClicked`）
- Info 区对齐 `SingleCardGrid`：`py-2.5 → pt-2.5 pb-1`，Row1 加 `min-h-[2.0625rem]`，Row3 `mt-1.5 → mt-0.5`
- 新增操作行：占位符（invisible Copy）/ FolderOpen / Trash2，布局与 SingleCardGrid 完全一致
- FolderOpen：`showItemInFolder(cover)` + 0.8s 绿色 flash 反馈
- 删除：内联确认条原地替换（逐个 `deleteFile` → `removeBatchHistory`）
- 空状态：`h-64 → flex-1 min-h-[16rem]`，大屏居中正常

*最后更新：2026-04-16（BatchCardGrid 高度对齐 + 操作按钮）*

---

### BatchCardGrid 视觉差异化 A+C（2026-04-16 完成，commit 919d3fd）

**目标：** 让批量任务卡片与单任务卡片在视觉上一眼可辨。

**A — 2×2 缩略图网格（`WorksPanel.tsx` `BatchCardGrid`）：**
- 取 `record.tasks` 中前4个有 `outputFile` 的任务组成网格（`coverTasks`）
- `coverTasks.length >= 2`：`grid grid-cols-2 grid-rows-2 gap-[2px]`，空余槽位用半透明占位 div
- `coverTasks.length === 1`：全尺寸单视频（与之前一致）
- `coverTasks.length === 0`：`Layers` 图标占位

**C — 更强层叠效果：**
- 后层（Layer 3）：`opacity-[0.55]`（原 0.30），`translate(8px, 8px)`（原 6px），边框 `border-brand/10`（原 rgba 0.05）
- 中层（Layer 2）：`opacity-[0.88]`（原 0.60），`translate(4px, 4px)`（原 3px），边框 `border-brand/[0.14]`（原 rgba 0.07）
- 前卡边框：`border-[rgba(255,255,255,0.18)]`（原 0.10）

---

### BatchCardGrid 自适应缩略图布局（2026-04-16 完成，commit a672632）

- 0 个输出：`Layers` 图标占位
- 1 个输出：全尺寸单视频
- 2 个输出：`grid-cols-2`（左右各半）
- 3 个输出：Instagram 三图布局（左半大图 + 右半上下两个）
- 4 个输出：`2×2` grid
- 永远没有空格占位，每个格子都是真实视频

---

### 作品卡信息区统一（2026-04-16 完成，commits 1777ad1–8f95b4e）

#### info 区结构对齐
- `BatchCardGrid` info 区调整为与 `SingleCardGrid` 完全一致的 3 行结构
- Row 3：仅日期（移除 ✓/✗ 数量，挪入缩略图 badge）
- `BatchCardGrid` 操作行移除 `justify-end`，按钮从左对齐（与单任务卡一致）
- 移除操作行 invisible 占位 span（历史遗留）
- `SingleCardGrid` Row 2 补充 aspectRatio 显示：`模型 · Ns · 16:9`
- `TaskRecord` 类型补充 `aspectRatio?: string` 字段

#### 缩略图左上角 badge 统一
最终 badge 结构（两种卡片一致）：
- pill 1（数量）：`单视频` / `N个视频`
- pill 2（状态，始终显示）：`已下载`（绿实底）/ `已完成`（暗底）/ `失败`/`N 失败`（红实底）

批量卡判断逻辑：
- `failCount > 0` → `N 失败`（优先级最高）
- `record.tasks.every(t => !!t.outputFile)` → `已下载`
- 否则 → `已完成`

---

### BatchDrawer 素材预览 + 任务行播放提示（2026-04-16 完成，commit eff4795）

- **素材点击**：图片素材由 `window.api.openFile`（系统文件查看器）改为 `setPreviewUrl(localFileUrlSync(path))`（产品内全览）
- **VideoModal 扩展**：通过正则检测图片扩展名（jpg/png/gif/webp/avif 等），命中则渲染 `<img>` 而非 `<video>`
- **BatchTaskRow hover 播放提示**：行 div 加 `group`，缩略图上叠加 Play 图标 overlay，hover 时淡入（`opacity-0 group-hover:opacity-100`）

*最后更新：2026-04-16（BatchDrawer 素材预览 + 任务行播放提示）*

---

### 失败卡片修复（2026-04-16 完成，commit 7cb3419）

**修复（`WorksPanel.tsx` + `electron.d.ts`）：**

#### 操作按钮左对齐
- `SingleCardGrid` 失败态：完全移除 Slot 1（copy）的 invisible 占位 div
- 之前 invisible wrapper 仍占用空间，导致 retry+delete 被推到右侧
- 现在失败卡只渲染 retry + delete，从左边缘对齐

#### 重试链路修复
- `handleRetry` 从同步改为 async，实际调用对应 API：
  - Seedance：调用 `window.api.runStructuredTask(task params)`，拿到 `submitId` 后调 `updateTask(id, { submitId })`
  - Kling O1：调用 `window.api.klingGenerate({ imagePaths, prompt, duration, aspectRatio, submitId })`
- App.tsx 通过 `submitId` 匹配进度事件，所以更新 `submitId` 是重试生效的关键
- 之前 `retryTask()` 只重置 status 为 pending，任务永远卡住

#### 缩略图区域透出失败原因
- 失败卡缩略图区域不再只显示 AlertTriangle 图标
- 改为：AlertTriangle 图标 + `parseTaskError().title`（error 标题）+ `parseTaskError().message`（完整原因）
- 若有 thumbnailUrl，以 `opacity-20` 的方式显示在背景，错误文字叠加在上方

#### 类型补充
- `electron.d.ts` 新增 `klingGenerate` 方法类型声明（之前缺失，仅在 preload.js 中存在）

---

### Skills UX 重设计（2026-04-16 完成，commits e05dee7–当前）

#### 4 张确认卡提取到 `src/components/confirm/`

| 文件 | 说明 |
|------|------|
| `VideoThumb.tsx` | Canvas 帧提取，共享组件 |
| `SaveSkillModal.tsx` | 保存/更新技能弹窗，自读 store |
| `SkillConfirmCard.tsx` | 技能确认卡，editable prompts + 点击 pills |
| `ConfirmCard.tsx` | AI 单任务确认卡，always-editable textarea + pills |
| `BatchConfirmCard.tsx` | 批量确认卡，pills + SaveSkillModal |
| `KlingConfirmCard.tsx` | 可灵确认卡，editable prompt + pills + SaveSkillModal |

#### 点击即用流程（pendingSkillConfirm）
- `store.ts` 新增 `pendingSkillConfirm: Skill | null` + `setPendingSkillConfirm`
- `SkillsPanel.tsx` `handleUse`：`setPendingSkillConfirm(skill)` 替换 `setActiveSkill`
- `ChatPanel.tsx` useEffect 监听 `pendingSkillConfirm`，自动插入 `skill-confirm` 消息，无需用户手动发送
- `handleApplySkill`（工具栏技能选择）直接插入确认卡消息，不再走 `handleSkillSend`
- 删除了 `handleSkillSend` 函数

#### SkillEditor 改进（`SkillsPanel.tsx`）
- 提示词 textarea 改为自动高度（`onInput` 动态计算，min 3.75rem / max 12.5rem）
- 素材槽位：有文件时显示缩略图（图片 `<img>`、视频 `VideoThumb`、音频 ♪），点击缩略图触发 VideoModal 预览

*最后更新：2026-04-16（Skills UX 重设计）*
